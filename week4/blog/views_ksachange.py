...
env = Environment(
    loader=FileSystemLoader('./blog/templates/jtemplates/',
                            followlinks=True))
...
def wallaceTreeMult(N,Modulename):
    ...
    top = templates_global_top.render(module=module)
    file_dump(Modulename,module)
#wallaceTreeMult(170,'trip')
...
def koggstone(N,Modulename):
    ...
    for j in range(1,round((math.log(N)/math.log(2))+1)+1):
        ...
        if(j<int((math.log(N)/math.log(2))+1)):
            a = a+"gcell g"+str(j)+"0 (cin,P"+str(j)+"["+str(2**(j-1)-1)+"],G"+str(j)+"["+str(2**(j-1)-1)+"],G"+str(j+1)+"["+str(2**(j-1)-1)+"]);\n"
        for k in range(1,2**(j-1)): 
            if(j>=3 and k==1 and j!=round((math.log(N)/math.log(2))+1)):
                a = a+"gcell g"+str(j)+str(k)+" (G2"+"["+str(k-1)+"], P"+str(j)+"["+str((2**(j-1))+k-1)+"], G"+str(j)+"["+str((2**(j-1))+k-1)+"], G"+str(j+1)+"["+str((2**(j-1))+k-1)+"]);\n"
            elif((2**(j-1))+k-1 < N):
                a = a+"gcell g"+str(j)+str(k)+" (G"+str(j)+"["+str(k-1)+"], P"+str(j)+"["+str((2**(j-1))+k-1)+"], G"+str(j)+"["+str((2**(j-1))+k-1)+"], G"+str(j+1)+"["+str((2**(j-1))+k-1)+"]);\n"
            else:
                a = a+"gcell g"+str(j)+"0 (cin,P"+str(j)+"["+str(2**(j-1)-1)+"],G"+str(j)+"["+str(2**(j-1)-1)+"],cout);\n"
                break
        for r in range(1,N-(2**(j-1)-1)-2**(j-1)+1):
            a = a+"bcell b"+str(j)+str(r)+" (G"+str(j)+"["+str(2**(j-1)+(r-2))+"], P"+str(j)+"["+str(2**(j)-1+(r-1))+"], G"+str(j)+"["+str(2**(j)-1+(r-1))+"], P"+str(j)+"["+str(2**(j-1)+(r-2))+"], G"+str(j+1)+"["+str(2**(j)-1+(r-1))+"], P"+str(j+1)+"["+str(2**(j)-1+(r-1))+"]);\n"
        a = a+"\n"
    b = ""
    for s in range(N):
    ...
    top = templates_global_top.render(module=context1,modulename=Modulename)
...
def top_gen_mult(N,Modulename):
    ...
    return
#def top_gen_adder(N,Modulename):
#    topp = templates_top_adder.render(modulename=Modulename)
#    file_dump_Adder('top',topp)
#    return
#koggstone(32,'roo')
#carry_look(30,'roo')
